        ...
        <section className="flex flex-col justify-center gap-5">
          <header className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-neutral-500">记录一组标准坐姿角度作为评分基线。</p>
            </div>
            <button
              onClick={() => navigate('/')}
            ...
            {(!sessionStarted || cameraError || modelError || modelLoading) && (
              <div className="absolute inset-0 flex items-center justify-center rounded-lg bg-neutral-950/80 p-6 backdrop-blur-sm">
                <div className="max-w-sm text-center">
                  <div className="mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-lg bg-neutral-900 text-cyan-300">
                    <Camera size={28} />
                  </div>
                  ...
                  {!cameraError && !modelError && !modelLoading && (
                    <p className="mb-5 text-sm leading-6 text-neutral-400">打开摄像头后开始校准。</p>
                  )}
                  <button
                    onClick={() => void startSession()}
                    disabled={modelLoading}
                    className="inline-flex items-center gap-2 rounded-md bg-cyan-400 px-5 py-2.5 text-sm font-semibold text-neutral-950 transition hover:bg-cyan-300 disabled:cursor-not-allowed disabled:bg-neutral-800 disabled:text-neutral-500"
                  >
